...
export const getJobApplications = async (query?: JobApplicationGetQueryInterface) => {
  const response = await axios.get(`/api/job-applications${query ? `?${queryString.stringify(query)}` : ''}`);
